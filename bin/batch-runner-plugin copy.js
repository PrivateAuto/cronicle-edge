 ...
 *
 * Requires: pixl-json-stream, pixl-tools, @aws-sdk/client-sts, @aws-sdk/client-codeartifact, unzipper, node-fetch
 */
...
stream.on('json', async (job) => {
  ...
  // Preserve newlines in command - don't trim to avoid losing intentional whitespace
  let command = String(params.command || '');
  // Only trim the outer whitespace, preserve internal newlines
  ...
  try {
    // CodeArtifact generic packages URL format
    const packageUrl = `https://${DOMAIN_NAME}-${ACCOUNT_ID}.d.codeartifact.${process.env.AWS_REGION || DEFAULT_REGION}.amazonaws.com/generic/${REPOSITORY_NAME}/${name}/${version}/${name}-${version}.zip`;
    const headers = {
      'Authorization': `Bearer ${token}`
    };
    const response = await fetch(packageUrl, { headers });
    if (!response.ok) throw new Error(`Failed to download package: ${response.status} ${response.statusText}`);
    
    const packageFile = fs.createWriteStream(packagePath + '.zip');
    await streamPipeline(response.body, packageFile);
    logAppend(job, `[Cronicle Batch] Downloaded package from CodeArtifact: ${packageUrl}`);
  } catch (e) {
  ...
  // 4) Extract package
  try {
    await fs.createReadStream(packagePath + '.zip').pipe(unzipper.Extract({ path: workDir })).promise();
    logAppend(job, `[Cronicle Batch] Extracted package into ${workDir}`);
  } catch (e) {
    return failWithCleanup(job, `Unzip failed: ${e.message}`, workDir);
  }
