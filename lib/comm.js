...
module.exports = Class.create({
	...
	pruneDeadWorkers: function() {
		// Remove workers that are no longer seen in discovery
		// This is called periodically by the manager to clean up workers that have disappeared
		if (!this.multi.manager) return;
		if (!this.Discovery || !this.Discovery.nearby) return;

		var nearbyServers = this.Discovery.getNearbyServers ? this.Discovery.getNearbyServers() : [];
		var nearbyHostnames = {};

		// Build map of currently visible servers
		for (var idx = 0; idx < nearbyServers.length; idx++) {
			nearbyHostnames[nearbyServers[idx].host] = true;
		}

		// Check each registered worker
		for (var hostname in this.workers) {
			var worker = this.workers[hostname];
			...
			// If worker is not in discovery layer and socket is disconnected/missing, remove it
			if (!nearbyHostnames[hostname]) {
				// Give it some grace period - only remove if socket is also gone
				if (!worker.socket || worker.socket.disconnected) {
					this.logDebug(5, "Pruning dead worker (not in discovery): " + hostname);
					this.removeServer({ hostname: hostname }, { reason: "Worker not seen in discovery heartbeat" });
				}
			}
		}
	},
