...
module.exports = Class.create({
	...
	safeJobLog: function(job) { // print less verbose, more readable job data on logging
		if(!job) return ''
		// in debug mod print all job data
		if(this.server.config.get('debug')) return job
		
		// clean up some parameters
		let params =  job.params || {}
		let paramString = 'params: ' + Object.keys(params).map(e=>{
			let val = params[e]
			if(e === 'script') return 'script=' + Buffer.from(String(val)).toString('base64') // to base64
			if(['headers', 'ssh_key', 'config'].includes(e)) return ''
			return e + '=' +  val
		}).join(';')

		let excl = ["table", "chain_data", "script", "secret", "env", "cat_secret", "plug_secret", "globalenv", "headers", "params", "html", "trend"]

		return paramString + '|' + Object.keys(job)
		   .filter(e => ! excl.includes(e))
		   .map(e => e + ': ' + ("workflow|perf|options".indexOf(e) > -1 ? JSON.stringify(job[e]) : job[e]) )
		   .join(" | ")
	},
	...
	tick: function () {
		...
		if (this.multi.manager) {
			...
			// Prune dead workers every 30 seconds
			if (!this.lastWorkerPrune || (now - this.lastWorkerPrune >= 30)) {
				this.lastWorkerPrune = now;
	...
	fireInfoHook: function (web_hook, data, logMessage) {
		...
		// try resolve hook info from webhooks property
		let namedWebhooks = self.server.config.get('web_hooks') || {}
		web_hook = namedWebhooks[web_hook] || web_hook
	...
	recoverJobLogs: function () {
		...
		glob(job_spec, {windows: process.platform === 'win32' }, function (err, files) {
			// got job json files

			async.eachSeries(files || [], function (file, callback) {
				// foreach job file
				let job_id = (typeof file === 'string' ? path.basename(file).match(/(\w+)(\-detached)?\.json$/) : [])[1]
				...
				function (err) {
					...
					glob(log_spec, {windows: process.platform === 'win32'}, function (err, files) {
						// got log files

						async.eachSeries(files || [], function (file, callback) {
							// foreach log file

							let job_id = (typeof file === 'string' ? path.basename(file).match(/(\w+)(\-detached)?\.log$/) : [])[1]
	...
	// 		if(err) return self.logDebug(2, 'Failed to fetch scheduled events for autostart', err)
			
	// 		events.forEach( evt => {
	// 			if(evt.repeat && evt.enabled) {
	// 				self.launchOrQueueJob(evt, (item, err)=>{
