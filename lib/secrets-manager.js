...
module.exports = class SecretsManager extends Component {
	...
	startup(callback) {
		...
		this.ssmClient = new SSMClient({
			region: awsConfig.region || 'us-east-2',
			...awsConfig
		});
	...
	async getParameter(parameterName, withDecryption = true) {
		try {
			// Ensure parameter name has leading slash
			const normalizedParamName = parameterName.startsWith('/') ? parameterName : `/${parameterName}`;

			// Check cache first
			const cacheKey = `${normalizedParamName}:${withDecryption}`;
			...
			this.logDebug(5, `Retrieved parameter from SSM: ${normalizedParamName}`);
			return value;

		} catch (error) {
			this.logError('ssm_parameter_error', `Failed to retrieve parameter ${normalizedParamName}: ${error.message}`);
			throw error;
		}
	}
	...
	async getParameters(parameterNames, withDecryption = true) {
		try {
			...
			for (const paramName of parameterNames) {
				// Ensure parameter name has leading slash
				const normalizedParamName = paramName.startsWith('/') ? paramName : `/${paramName}`;
				const cacheKey = `${normalizedParamName}:${withDecryption}`;
				const cached = this.parameterCache.get(cacheKey);

				if (cached && (Date.now() - cached.timestamp) < (this.cacheTimeout * 1000)) {
					cachedResults[normalizedParamName] = cached.value;
				} else {
					uncachedParams.push(normalizedParamName);
				}
			}
	...
	async resolveConfigSecrets(config) {
		try {
			...
			// Extract unique parameter names and ensure they have leading slashes
			const parameterNames = [...new Set(secretRefs.map(ref => {
				const paramName = ref.parameterName;
			...
			for (const ref of secretRefs) {
				const normalizedParamName = ref.parameterName.startsWith('/') ? ref.parameterName : `/${ref.parameterName}`;
				if (parameters[normalizedParamName]) {
					this.setNestedValue(resolvedConfig, ref.path, parameters[normalizedParamName]);
