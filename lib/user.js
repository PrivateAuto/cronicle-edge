...
module.exports = Class.create({
	...
	defaultConfig: {
		...
		"oauth_profiles": {
		    ...
		    "workos": {
				...
				"user_url": "https://api.workos.com/sso/profile",
				"user_attribute": "email",
				"avatar_attribute": "profile_picture_url",
	...
	do_ldap_auth: async function(username, password, returnUserObject, verifyGroup) { //
		...
		try {
			...
			let userObject =  {
				...
				privileges: {admin: 0, run_events:0, abort_events:0, edit_events:0, create_events: 0, delete_events: 0, state_update: 0, disable_enable_events: 0, cat_limit: 0, grp_limit:0},
			...
			let namedRoles = {
				admin:   {admin: 1, run_events:1, abort_events:1, edit_events:1, create_events: 1, delete_events: 1, state_update: 1, disable_enable_events: 0, cat_limit: 0, grp_limit:0},
				editor:  {admin: 0, run_events:1, abort_events:1, edit_events:1, create_events: 1, delete_events: 1, state_update: 1, disable_enable_events: 1, cat_limit: 0, grp_limit:0},
				runner:  {admin: 0, run_events:1, abort_events:1, edit_events:0, create_events: 0, delete_events: 0, state_update: 0, disable_enable_events: 0, cat_limit: 0, grp_limit:0},
				// viewer:  {admin: 0, run_events:0, abort_events:0, edit_events:0, create_events: 0, delete_events: 0, state_update: 0, disable_enable_events: 0, cat_limit: 0, grp_limit:0},
			}
	
			let validPrivs = ['admin', 'run_events', 'abort_events', 'create_events', 'edit_events', 'delete_events', 'disable_enable_events', 'cat_limit', 'grp_limit']
			...
			if (groupConf.mapping && (typeof groupConf.mapping !== 'string') && (!Array.isArray(groupConf.mapping))) {
				...
				for(let key in groupConf.mapping) {
					...
					groupMap[grp.toUpperCase()] = groupConf.mapping[key]
				}
				...
				for (let g = 0; g < userGroups.length; g++) {
					...
					if(typeof groupName === 'string' ) groupName = groupName.toUpperCase()
					else { continue }
	...
	// helper to join base_path with given location
	getBaseLocation: function (subPath) {

		if (typeof subPath !== 'string') subPath = '/'

		let basePath = this.server.config.get('base_path')

		if (/^\/\w+$/.test(basePath)) { // if valid subpath like /cron
			if (subPath === '/') return basePath
			if (subPath.startsWith('/')) return (basePath + subPath)
	...
	api_oauth: function (args, callback) {
		...
		if(session_id) {
			...
			res.writeHead(302, { Location: self.getBaseLocation('#' + orig_location) })
			res.end()			
		}
		...
		self.fireHook('before_login', args, function (err) {
			...
			self.storage.put('sessions/' + session_id, session, function (err, data) {
				if (err) {
					...
					let res = args.response
					let base_path = self.getBaseLocation('/')
					res.setHeader('Set-Cookie', `session_id=${session_id}; HttpOnly; Path=${base_path}; Max-Age=10; Secure=True; SameSite=Strict`)
					res.writeHead(302, { Location: self.getBaseLocation('#' + orig_location) })
					res.end()			
	...
	api_logout: function (args, callback) {
		...
		args.response.setHeader('Set-Cookie', `session_id=; HttpOnly; Path=${self.getBaseLocation('/')}; Max-Age=0; Secure=True; SameSite=Strict`);
